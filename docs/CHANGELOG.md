...
### v2.3.1 - Railway 배포 수정 (2024-12-07)

#### 🐛 버그 수정

##### Railway Health Check 실패 문제
- **증상**: 2일간 23건의 배포 에러 발생
- **원인**: 
  1. `ENV PORT=8000`이 Railway의 동적 포트를 덮어씀
  2. `@app.on_event("startup")`이 deprecated

- **수정 내용**:

**Dockerfile 수정:**
```dockerfile
# 이전 - ENV PORT=8000이 Railway 포트를 덮어씀
ENV PORT=8000
CMD uvicorn main:app --host 0.0.0.0 --port $PORT

# 변경 후 - Railway가 제공하는 $PORT 사용
CMD ["sh", "-c", "uvicorn main:app --host 0.0.0.0 --port ${PORT:-8000}"]
```

**main.py 수정:**
```python
# deprecated된 on_event 제거
# @app.on_event("startup") → 간단한 초기화 로그로 대체
```

##### ⚠️ 모니터링 필요
- Railway 배포 상태 주시 필요
- Health check 실패 시 로그 확인
